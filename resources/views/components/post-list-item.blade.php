<div class="flex max-w-3xl flex-col space-y-16">
    <article class="md:grid md:grid-cols-4 md:items-baseline py-8">
        <div class="md:col-span-3 group relative flex flex-col items-start">
            ...
            <time datetime="{{ $post->date() }}" class="md:hidden relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400 pl-3.5">
        ...
        <time class="mt-1 hidden md:block relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400" datetime="{{ $post->date() }}">
            {{ $post->date()->diffForHumans() }}
        </time>
    </article>
</div>
